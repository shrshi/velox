...
/// Join operators always run on CPU because the cudf custom join bridge
/// is incompatible with mixed grouped execution.  Results are verified
/// against DuckDB.
///
/// Usage:
...
int main(int argc, char** argv) {
  ...
  std::cout << "DEBUG: folly::Init done" << std::endl;
  exec::test::setupMemory(FLAGS_allocator_capacity, FLAGS_arbitrator_capacity);
  std::cout << "DEBUG: setupMemory done" << std::endl;
  ...
  std::cout << "DEBUG: calling registerCudf" << std::endl;
  cudf_velox::registerCudf();
  std::cout << "DEBUG: registerCudf done" << std::endl;

  std::shared_ptr<memory::MemoryPool> rootPool{
      memory::memoryManager()->addRootPool()};
  std::cout << "DEBUG: rootPool created" << std::endl;
  ...
  std::cout << "DEBUG: referenceQueryRunner created" << std::endl;
  ...
  std::cout << "DEBUG: all serializers registered, calling joinFuzzer"
            << std::endl;
